...
**415 models.** Grouped by input, because that is the choice you make first — a model that
...
| `aion-3.5` | aionlabs | — | 262k ctx, 33k out, streaming, tools, json mode · Aion 3.5 is a multi-model roleplaying and storytelling system from AionLabs, built on the GLM family of models. |
| `aion-3.5-mini` | aionlabs | — | 262k ctx, 33k out, streaming, tools, json mode · Aion 3.5 Mini is a multi-model roleplaying and storytelling system from AionLabs, built on the GLM family of models. |
...
| `claude-opus-5.5` | anthropic | — | 1000k ctx, 128k out, streaming, tools, vision, pdf, json mode · Claude Opus 5.5 is Anthropic&#x27;s flagship model for demanding reasoning, coding, and long-horizon agentic work, succeeding Claude Opus 5. |
...
| `command-a-plus` | cohere | — | 192k ctx, 64k out, streaming, tools, vision, json mode · Command A+ is Cohere&#x27;s flagship model for enterprise agentic workflows. 192,000 token context window, maximum output of 64,000 tokens. |
...
| `deepseek-flash-latest` | deepseek | — | 1049k ctx, 944k out, streaming, tools, vision, json mode · This model always redirects to the latest model in the DeepSeek Flash family. |
| `deepseek-pro-latest` | deepseek | — | 1049k ctx, 944k out, streaming, tools, json mode · This model always redirects to the latest model in the DeepSeek Pro family. |
...
| `deepseek-v4.1-flash` | deepseek | — | 1049k ctx, 384k out, streaming, tools, vision, json mode · DeepSeek V4.1 Flash is a sparse mixture-of-experts model from DeepSeek, and the cost-efficient tier of the V4.1 family. |
...
| `ember-1` | fireworks | — | 1049k ctx, 944k out, streaming, tools, vision, json mode · Ember-1 is a specialized reasoning model from Fireworks Research, built on Kimi K3. 1,048,576 token context window. |
...
| `mercury-2.5` | inception | — | 260k ctx, 66k out, streaming, tools, json mode · Mercury 2.5 is the fastest reasoning LLM, and the latest diffusion LLM (dLLM) from Inception. |
...
| `ling-3.0-flash-vl` | inclusionai | — | 131k ctx, 33k out, streaming, tools, vision, json mode · Ling 3.0 Flash VL builds on Ling 3.0 Flash (124B total / 5.5B active MoE from InclusionAI), further strengthening its language capabilities… |
| `schematron-v2-small` | inference-net | — | 128k ctx, 4k out, streaming, json mode · Schematron V2 Small is a 3B-parameter HTML-to-JSON extraction model from Inference.net. |
| `schematron-v2-turbo` | inference-net | — | 128k ctx, 8k out, streaming, json mode · Schematron V2 Turbo is a 3B-parameter HTML-to-JSON extraction model from Inference.net. |
...
| `gpt-6-luna` | openai | — | 1050k ctx, 128k out, streaming, tools, vision, pdf, json mode · GPT-6 Luna is the fast, cost-efficient model in OpenAI&#x27;s GPT-6 series, positioned below GPT-6 Sol. |
| `gpt-6-luna-pro` | openai | — | 1050k ctx, 128k out, streaming, tools, vision, pdf, json mode · GPT-6 Luna Pro is the same underlying model as GPT-6 Luna, served with reasoning.mode set to pro for higher-quality responses on complex… |
| `gpt-6-sol` | openai | — | 1050k ctx, 128k out, streaming, tools, vision, pdf, json mode · GPT-6 Sol is the cost-efficient high-end model in OpenAI&#x27;s GPT-6 series, positioned below the flagship GPT-6 Astra and above the fast… |
| `gpt-6-sol-pro` | openai | — | 1050k ctx, 128k out, streaming, tools, vision, pdf, json mode · GPT-6 Sol Pro is the same underlying model as GPT-6 Sol, served with reasoning.mode set to pro for higher-quality responses on complex… |
| `gpt-astra-latest` | openai | — | 1050k ctx, 128k out, streaming, tools, vision, pdf, json mode · This model always redirects to the latest model in the OpenAI GPT Astra family. |
...
| `gpt-luna-latest` | openai | — | 1050k ctx, 128k out, streaming, tools, vision, pdf, json mode · This model always redirects to the latest model in the OpenAI GPT Luna family. |
...
| `gpt-sol-latest` | openai | — | 1050k ctx, 128k out, streaming, tools, vision, pdf, json mode · This model always redirects to the latest model in the OpenAI GPT Sol family. |
| `gpt-terra-latest` | openai | — | 1050k ctx, 128k out, streaming, tools, vision, pdf, json mode · This model always redirects to the latest model in the OpenAI GPT Terra family. |
...
| `perceptron-mk1.5` | perceptron | — | 37k ctx, 8k out, streaming, tools, vision · Perceptron Mk1.5 is Perceptron&#x27;s embodied reasoning model for physical agents. |
...
| `ternary-bonsai-2-27b` | prism-ml | — | 262k ctx, 33k out, streaming, tools, vision, json mode · Bonsai 2 27B is a 27B-parameter reasoning model from PrismML derived from Qwen3.8-27B. |
...
| `qwen3.8-max-0902` | qwen | — | 1000k ctx, 131k out, streaming, tools, vision, json mode · Qwen3.8 Max 0902 is an updated snapshot of Qwen3.8 Max from Alibaba&#x27;s Qwen team. |
| `qwen3.8-max-prime` | qwen | — | 1000k ctx, 131k out, streaming, tools, vision, json mode · Qwen3.8 Max Prime is a higher-throughput variant of Qwen3.8 Max from Alibaba&#x27;s Qwen team, served as a separate SKU at a higher price… |
| `qwen3.8-omni-flash` | qwen | — | 1000k ctx, 131k out, streaming, tools, vision, json mode · Qwen3.8 Omni Flash is an omni-modal reasoning model from Alibaba, the first Qwen model built around agentic capabilities with native… |
...
| `fugu-max` | sakana | — | 1000k ctx, 128k out, streaming, tools, vision, pdf · Fugu Max is the cost-performance model in Sakana AI&#x27;s Fugu family. 1,000,000 token context window, maximum output of 128,000 tokens. |
...
| `fugu-ultra-v2` | sakana | — | 1000k ctx, 128k out, streaming, tools, vision, pdf · Fugu Ultra v2 is the higher-performance model in Sakana AI&#x27;s Fugu family. |
...
| `space-bunny-alpha` | stealth | — | 1000k ctx, 524k out, streaming, tools, vision, json mode · Space Bunny Alpha is an anonymous large model with blazing-fast inference, strong coding capabilities and native multimodal input support. |
...
| `tev1-4b-experimental` | together | — | 33k ctx, streaming |
| `jev-router` | typesafe | — | streaming, vision, pdf · Jev Router picks the best model and reasoning effort for each request, balancing quality, speed, and cost. This model is free to use. |
| `pareto` | unbiased | — | 262k ctx, 131k out, streaming, tools, vision · Pareto is a multimodal composite model built for research, coding, and agentic workflows, while delivering frontier-level performance… |
...
| `solar-mini4` | upstage | — | 524k ctx, 131k out, streaming, tools, json mode · Solar Mini 4 is Upstage&#x27;s compact, cost-efficient language model, a 35B-parameter mixture-of-experts with 3B active parameters and a… |
...
| `grok-4.7` | xai | — | 500k ctx, 450k out, streaming, tools, vision, pdf, json mode · Grok 4.7 is SpaceXAI&#x27;s flagship model for coding, agentic tasks, and knowledge work, succeeding Grok 4.6. 500,000 token context window. |
...
| `mimo-v2.6-flash` | xiaomi | — | 1049k ctx, 131k out, streaming, tools, vision, json mode · MiMo-V2.6-Flash is an open-source foundation model developed by Xiaomi. 1,048,576 token context window, maximum output of 131,072 tokens. |
| `mimo-v2.6-pro` | xiaomi | — | 1049k ctx, 131k out, streaming, tools, vision, json mode · MiMo-V2.6-Pro is the flagship foundation model developed by Xiaomi. 1,048,576 token context window, maximum output of 131,072 tokens. |
| `mimo-v2.6-pro-ultraspeed` | xiaomi | — | 1049k ctx, 131k out, streaming, tools, vision, json mode · MiMo-V2.6-Pro-UltraSpeed is the fast speed edition of Xiaomi&#x27;s flagship foundation model, MiMo-V2.6-Pro. |
...
| `glm-5.3-flashx` | z-ai | — | 1049k ctx, 131k out, streaming, tools, vision, json mode · GLM-5.3-FlashX is the high-speed variant of Z.ai&#x27;s GLM-5.3-Flash, a native multimodal model delivering inference speeds of up to 200… |
| `glm-5.3-prime` | z-ai | — | 1000k ctx, 131k out, streaming, tools, json mode · GLM-5.3-Prime is the high-speed variant of Z.ai&#x27;s GLM-5.3, inheriting its full capabilities while delivering 1.5–2× the output… |
