...
| `embed-product` | bria | — | Seamlessly embed products into any scene with pixel-perfect control, automatic perspective, and natural lighting. |
| `fibo-edit-1.5` | bria | — | Commercially safe, multi-reference image editing model. |
| `fibo-edit-1.5-product-holding` | bria | — | Bria Product Holding edits a person photo to show the subject holding or carrying a product, using one to three product reference images… |
| `fibo-edit-1.5-virtual-try-on` | bria | — | Bria Virtual Try-On edits a person photo to show the subject wearing garments or accessories from one to three reference images, guided by… |
...
| `increase-resolution` | bria | — | Upscales image resolution to improve sharpness and size while preserving the original content |
...
| `seedream-v5-flash-layerize` | bytedance | — | Seedream 5.0 Flash is a fast image generation and editing model, built for workflows where speed and budget matter. |
...
| `virtual-try-on` | google | — | Generate realistic virtual try-on images from a person image and a clothing product image. |
...
| `recraft-v4-styles-pro-svg` | recraft-ai | — | Generate high-quality editable SVGs with Recraft V4 Styles Pro SVG. |
...
| `flux-srpo` | black-forest-labs | Image | FLUX.1 SRPO [dev] is a 12 billion parameter flow transformer that generates high-quality images from text with incredible aesthetics. |
...
| `fibo-gen-1.5` | bria | — | Text-to-image model with high-fidelity outputs, accurate typography, and style preset, strong in photorealism, textures, and beyond. |
...
| `seedream-v5-flash` | bytedance | Image | Seedream 5.0 Flash is a fast image generation and editing model, built for workflows where speed and budget matter. |
...
| `muse-image` | meta | Image | Meta's Muse Image model has faithful instruction-following and exceptional visual fidelity, with fine details like text, plots, and QR… |
...
| `arrow-1.1` | quiverai | — | Generate clean, editable SVGs from text prompts. |
| `arrow-1.1-max` | quiverai | — | Generate high-fidelity, detailed SVGs from text prompts. |
| `qwen/qwen-image` | qwen | Image | Qwen Image open source model - fast, highly capable image generation model from Alibaba Qwen |
| `recraft-v4` | recraft | — | Recraft V4 generates art-directed images with strong composition, accurate text rendering, and design taste built in. |
...
| `v4-style` | recraft | — | Generates raster images that hold a consistent style, from either a saved style ID or reference images attached directly. |
| `v4-style-pro` | recraft | — | Generates raster images that hold a consistent style, from either a saved style ID or reference images attached directly. |
| `v4-style-pro-text-to-vector` | recraft | — | Generates vector images that hold a consistent style, from either a saved style ID or reference images attached directly. |
| `v4-style-text-to-vector` | recraft | — | Generates vector images that hold a consistent style, from either a saved style ID or reference images attached directly. |
| `v4.1-flash` | recraft | — | Recraft V4.1 Flash generates raster images from text prompts, including photography, illustrations, and mixed-media compositions, with… |
