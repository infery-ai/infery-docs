...
**404 models.** Grouped by input, because that is the choice you make first — a model that
...
| `happy-horse-text-to-video` | alibaba | — | Generate 1080p video with synchronized native audio from a text prompt. Aspect ratios: 16:9, 9:16, 1:1, 4:3, 3:4. Duration: 3–15s. |
...
| `wan/v2.6/text-to-video` | alibaba | — | Wan 2.6 text-to-video model. |
...
| `bytedance/seedance-2.0/us/reference-to-video` | bytedance | Audio, Image, Video | US hosted version of ByteDance's most advanced reference-to-video model. |
| `bytedance/seedance-2.5/us/text-to-video` | bytedance | — | US-hosted ByteDance Seedance 2.5 generates cinematic video from text with synchronized audio, up to 30-second duration, and 480p or 720p… |
...
| `seedance-2.0` | bytedance | Image, Video, Audio | Seedance 2.0 by ByteDance generates high-quality video with synchronized audio from text, images, video, and audio inputs. |
| `seedance-2.0-fast-reference-to-video` | bytedance | Image, Video, Audio | ByteDance's most advanced reference-to-video model, fast tier. |
...
| `seedance-2.0-us` | bytedance | Image | US hosted version of ByteDance's most advanced text-to-video model. |
...
| `seedance-2.5-us-reference-to-video` | bytedance | Audio, Image, Video | US-hosted ByteDance Seedance 2.5 generates video with native audio from up to 30 images, 10 videos, and 10 audio references. |
...
| `agent-ray-v3.2-video-to-video` | luma | Image, Video | Luma Ray 3.2 re-renders an existing video into new cinematic motion guided by a text prompt, preserving the source's look and movement… |
...
| `h3-max-styles-16bit-pixel` | minimax | — | Generates 768p video with audio in a 16-bit pixel-art style from text prompts or an optional first-frame image. |
| `h3-max-styles-hand-drawn` | minimax | — | Generates 768p video with audio in a hand-drawn animation style from text prompts or an optional first-frame image. |
| `h3-max-styles-low-poly` | minimax | — | Generates 768p video with audio in a retro low-poly 3D style from text prompts or an optional first-frame image. |
| `h3-max-styles-retro-toon-70s` | minimax | — | Generates 768p video with audio in a retro 1970s hand-painted animation style from text prompts or an optional first-frame image. |
| `h3-max-styles-vhs` | minimax | — | Generates 768p VHS-style video with audio from text prompts or an optional first-frame image. |
...
| `flux-3-edit-video` | blackforestlabs | — | FLUX.3 is Black Forest Labs' frontier video model. |
...
| `minimax/h3-max/extend-video` | minimax | — | H3 Max Extend Video adds a text-guided continuation to an existing video. |
...
| `flux-3-action-so101` | black-forest-labs | Video | FLUX 3 Action turns what the robot sees into what it does next. |
...
| `bytedance/seedance-2.5/us/image-to-video` | bytedance | — | US-hosted ByteDance Seedance 2.5 animates still images with synchronized audio and optional end-frame control. |
...
| `id-v2v` | id-v2v | Video | Restyle a video’s scene, lighting, and visual style from edited keyframes while preserving the source subjects’ identity, expressions… |
| `id-v2v-relight` | id-v2v | Video | Change a video’s lighting using a relit reference frame while preserving the scene, subjects, and original performance. |
...
| `h3-max-camera-controls` | minimax | — | H3 Max Multi Angle turns a single image into a video with precise, keyframe-based control over the camera's orbit, elevation, and distance… |
| `h3-max-lip-sync` | minimax | Audio | H3 Max Lip Sync generates a video from an image and supplied audio, synchronizing mouth movements to the soundtrack. |
...
| `music-video-vibemv` | pixverse | — | PixVerse VibeMV generates music videos from audio, with optional character references and lyric subtitles. |
