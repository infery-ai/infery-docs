...
<Frame caption="The run bar: what this workflow would cost, and the button that spends it">
  <img src="/samples/workflow-run-bar.png" alt="A bar across the bottom of the canvas reading Est. (canvas) approximately 2.88 to 4.16 cr, with a green Run button on the right." />
</Frame>
...
<Frame caption="A succeeded run: every step's result, cost and duration, still on the canvas">
  <img src="/samples/workflow-run-result.png" alt="The workflow editor showing a finished run of a workflow called Watermarked social set. A banner reads: showing the succeeded run from 14 Aug 16:34, the canvas is v1, the version it ran, which is also the published one, Save publishes this canvas as v2. Two input cards feed a gpt-image-2 step that produced a logo, an image.resize step, and three parallel image.pipeline steps each carrying two operations and showing its own output thumbnail. The Output node lists named results MARK, WIDE and STORY, and one more. The bar along the bottom reads RUN Succeeded, Est. approximately 11 cr, 0.5 cr, then per-step entries: image.resize 0.125cr 1166ms, media 0cr 0ms, and three image.pipeline entries at 0.125cr and around 1.2 seconds each." />
</Frame>
...
| `pipeline_timeout_seconds` in a definition | values above 120 are refused |
