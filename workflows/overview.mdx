...
<Frame caption="The two ways to start">
  <img src="/samples/workflow-new.png" alt="A dialog headed New workflow with two options: Blank, start with an empty workflow, and Browse templates, start pre-filled from a template." />
</Frame>

**Blank** gives you a canvas with nothing on it but an output node. You name it and
start adding steps.
...
<Frame caption="The template gallery: each card names the step types it uses">
  <img src="/samples/workflow-templates.png" alt="A gallery of template cards under category chips reading All, audio, content, data, document, localization, research and video. Cards include Video to thumbnail, PDF to AI summary, Web research to summary, Research brief from a URL, Article to social thread and Blog post from a URL. Each card carries a description and small tags naming the step types it uses, such as web.search, model, http and foreach." />
</Frame>
...
| `pipeline_timeout_seconds` | no | Positive integer. Values above **120** are refused with `timeout_too_large`. |
...
<CardGroup cols={2}>
  ...
  <Card title="Step types" icon="cube" href="/workflows/step-types">
    `model`, `media`, `http`, `sub_pipeline` and the 31 capabilities.
  </Card>
