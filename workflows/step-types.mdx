...
A definition has six flow-control step types — `model`, `media`, `http`, `sub_pipeline`,
`foreach` and `parallel` — plus one step type per **capability** (`image.resize`,
`video.trim`, `web.search`, …). The two containers have [their own page](/workflows/containers).
...
<Frame caption="The values a run supplies">
  <img src="/samples/palette-input.png" alt="A palette group headed INPUT with five entries: Text, for prompts, scripts and lyrics; Image, PNG, JPG and reference images; Video, MP4, MOV and source footage; Audio, MP3, WAV and sound files; and File, any other uploaded file." />
</Frame>
...
<Frame caption="The steps that call a model">
  <img src="/samples/palette-generators.png" alt="A palette group headed GENERATORS with five entries: Image Generator, text or reference to image; Video Generator, text or image to video; Audio Generator, speech, music and sound effects; 3D Generator, image to 3D model; and LLM, text and vision models." />
</Frame>

**LLM** is the [`model`](#model) step. The four generators are
[`media`](#media) steps, one per modality.

### Media tools

Deterministic work on files: no model, no prompt, a fixed price. Each entry is a
[capability step](#capability-steps) — `image.resize`, `video.trim`,
`audio.concat` and so on.

<Frame caption="Image tools">
  <img src="/samples/palette-image.png" alt="A palette group headed IMAGE TOOLS listing nine entries: Resize image, Crop image, Rotate image, Convert image format, Composite (overlay) images, Add padding, Flatten alpha, Add alpha channel, and Multi-op image pipeline, which chains multiple image operations in one step." />
</Frame>

<Frame caption="Video tools">
  <img src="/samples/palette-video.png" alt="A palette group headed VIDEO TOOLS listing ten entries: Convert video format, Trim video, Concatenate videos, Scale video dimensions, Crop video frame, Overlay watermark, Extract single frame, Extract audio track, Multi-op video pipeline chaining up to ten operations, and Add audio to video." />
</Frame>

<Frame caption="Audio tools">
  <img src="/samples/palette-audio.png" alt="A palette group headed AUDIO TOOLS listing five entries: Transcribe, audio to text; Convert audio format; Trim audio; Concatenate audio; and Multi-op audio pipeline." />
</Frame>
...
<Frame caption="Document tools">
  <img src="/samples/palette-document.png" alt="A palette group headed DOCUMENT TOOLS with three entries: Extract text from document, parsing PDF, DOCX, HTML and others to markdown or JSON; Convert document format, between markdown, html, docx, pdf and epub; and Render markdown slides, converting markdown to a slide deck as pdf, pptx or html." />
</Frame>

<Frame caption="Archive tools">
  <img src="/samples/palette-archive.png" alt="A palette group headed ARCHIVE TOOLS with two entries: Create archive, packing files into zip or tar.gz; and Extract archive, unpacking zip, tar.gz or rar." />
</Frame>

<Frame caption="Web">
  <img src="/samples/palette-web.png" alt="A palette group headed WEB with two entries: HTTP, call an external endpoint; and Web search, which returns a grounded answer plus more." />
</Frame>
...
<Frame caption="Code">
  <img src="/samples/palette-code.png" alt="A palette group headed CODE with two entries: Run Python code and Run Node.js code, each executing in a sandboxed runtime." />
</Frame>

### Flow

<Frame caption="Steps that hold other steps">
  <img src="/samples/palette-flow.png" alt="A palette group headed FLOW with three entries: Loop, iterate over a list; Parallel, race branches where the first success wins; and Sub-workflow, call another workflow." />
</Frame>
...
<Frame caption="Annotations, not steps">
  <img src="/samples/palette-canvas.png" alt="A palette group headed CANVAS with two entries: Sticky Note, for notes and annotations; and Group, to frame a phase of the workflow." />
</Frame>
