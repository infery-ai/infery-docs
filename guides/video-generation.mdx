...
**Publishing to a platform?** Each placement carries its own shape *and* its own
duration ceiling, and they are not close to each other — a Reel allows 900
seconds, TikTok and YouTube Shorts 180, an X video 140.
[Publishing formats](/guides/publishing-formats) lists every one, so you can name
the placement and let the ceiling be applied for you instead of looking it up.
